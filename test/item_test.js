...
describe('Item',function(){
  ...
  it('should be able to get name',function(done){
    let item = new Item.Item('bronze sword', 'a bit dull', item_image);
  ...
  it('should be able to validate',function(done){
    //null name -> fail
    let item = new Item.Item('', 'a bit dull', item_image);
    expect(function(){item.validate();}).toThrow(/name/);

    //null desc -> pass
    item = new Item.Item('bronzeSword', '', item_image);
    expect(item.validate()).toBe(true);

    //null image -> pass
    item = new Item.Item('bronzeSword', 'a bit dull', '');
    expect(item.validate()).toBe(true);

    //symbols name -> fail
    item = new Item.Item('BronzeSw()rd', 'a bit dull', item_image);
    expect(function(){item.validate();}).toThrow(/name/);

    //symbols desc -> pass
    item = new Item.Item('bronze sword', 'a bit dull!', item_image);
    expect(item.validate()).toBe(true);

    //length name in (3-30 chars) -> pass else fail
    item = new Item.Item('QQ', 'a bit dull', item_image);
    expect(function(){item.validate();}).toThrow(/name/);

    item = new Item.Item('This is an item that is sharp and bronze maybe a sword', 'a bit dull', item_image);
    expect(function(){item.validate();}).toThrow(/name/);

    //length desc < 150 chars -> pass else fail
    item = new Item.Item('bronze sword', 'This is an item that is sharp and bronze maybe a sword', item_image); //pass
    expect(item.validate()).toBe(true);
    ...
    item = new Item.Item('bronze sword', desc, item_image); //fail
    ...
    item = new Item.Item('bronze sword', 'a bit dull', url);
