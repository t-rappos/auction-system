...
function getListing(listingId){
  return DB.getSingle(Queries.getListing(listingId), fillListing);
}
...
//TODO: make sure each listing is processed as cancelled, or removed from queue...
  function cancelAllListings(){
    return new Promise((resolve, reject)=>{
      ...
      .then(()=>{
        Utility.logWarning("cancelAllListings: TODO: call _processCancelledListingFn");
        _expiryValid = [];
        resolve();
      })
      .catch((e)=>{
  ...
  function cancelAccountListings(accountId){
    return new Promise((resolve, reject)=>{
      ...
      .then(()=>{
        Utility.logWarning("cancelAccountListings: TODO: unqueue listings for account");
        resolve();
      })
      .catch((e)=>{
        Utility.logError(e);
