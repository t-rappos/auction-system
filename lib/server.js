...
io.on('connection', function(socket){
  ...
  socket.on('view_items', function(callback){
    ...
    let ztags = [];
    let zimages = [];
    Promise.all([ItemFacade.createImage('image1','./assets/image1.jpg'),
                 AccountFacade.getAccount(username)])
    .spread((image, account)=>{
      accountId = account.id;
      return ItemFacade.createItem(image.id, 'zztestItem', 'testDesc', null, account.id);
    })
    .then((item)=>{
      return Promise.all([TagFacade.createItemTagValue(item.id,'value',Math.random()*1000.00),
                          TagFacade.createItemTagValue(item.id,'quantity',Math.random()*15)]);
    })
    .then(()=>{
