...
//options.itemId
  ...
  socket.on('listing_item_bid', function(options, callback){
  ...
  socket.on('view_account_listings', function(options, callback){
    ListingFacade.getAccountListings(options.accountId)
    .then((listings)=>{
      callback({listings});
    })
    .catch((e)=>{
  ...
  socket.on('view_messages', function(callback){
    ...
    .then((messages)=>{
      if(messages.length > 0){
        ...
        return [messages, AccountFacade.getAccountNameIds(messageSenderIds)];
      }
    })
    .spread((messages, senders)=>{
      console.log('returning ' + messages.length + " messages to client");
      console.log('returning ' + senders.length + " sender name/ids to client");
