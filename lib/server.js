...
//options.itemId
  ...
  function getListingsData(listingFunc,callback){
     let zimages = [];
    let ztags = [];
    let zlistings = [];
    ItemFacade.getImageList()
    ...
    .spread((maxBids, tagValues)=>{
      console.log('returned max bids ', maxBids);
  ...
  socket.on('view_account_listings', function(options, callback){
    return new Promise((resolve, reject)=>{
      ...
      AccountFacade.getAccount(username)
      .then((account)=>{
        return getListingsData(ListingFacade.getAccountListings.bind(null,account.id),callback);
      })
      .catch((e)=>{
        console.log(e);
      callback({error : e});
  ...
  socket.on('view_listings', function(options, callback){
    return getListingsData(ListingFacade.getAllListings,callback);
  });
