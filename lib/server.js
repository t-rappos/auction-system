...
//options.itemId
  ...
  socket.on('buyout_listing', function(options, callback){
    ...
    .then((res)=>{
      console.log('buyout listing resolved ->');
      console.log(res);
      let ls= UserFacade.getSocket(res.listerId);
      let ln = UserFacade.getUsername(ls);
      if(ls){
        ls.emit('item_sold');
      }
      console.log("Item sold by " + username + " to user " + ln);
      callback({});
    })
    .catch((e)=>{
      console.log(e);
  ...
  socket.on('view_bids', function(options, callback){
      return new Promise((resolve, reject)=>{
        ...
        .then((bids)=>{
          let listings = [];
          bids.map((b)=>{if(b && b.listing){listings.push(b.listing);}});
  ...
  socket.on('get_account_id',function(options, callback){
    ...
    .catch((e)=>{
      console.log("account check id throw error " + e);
      callback({error : e});
    });
  });
...
*/
  socket.on('create_message', function(options, callback){
    ...
    let id = UserFacade.getId(socket);
    AccountFacade.sendMessage(id, options.recipientId, options.title, options.message)
    .then(()=>{
      callback(true);
      let recipientSocket = UserFacade.getSocket(options.recipientId);
      if(recipientSocket){recipientSocket.emit('new_message');}
    })
    .catch((e)=>{
      console.log(e);
      callback({error : e});
