...
//initialiseServer()
//.then(()=>{
    /*let expiredListingHandlerId = */setInterval(()=>{processAllExpiredListings();},60000);
//});
...
function processExpiredListing(listing){
    ...
    //}
    return new Promise((resolve, reject)=>{
        let UserFacade = require('./user.js');
        getHighestBidderTransaction(listing.id)
        .then((maxTrans)=>{
            if(maxTrans!=null){
                let sellerSocket = UserFacade.getSocket(listing.sellerId);
                let bidderSocket = UserFacade.getSocket(maxTrans.bidderId);
                if(sellerSocket){sellerSocket.emit('item_sold');}
                if(bidderSocket){bidderSocket.emit('item_won');}
                return Promise.all([
                ...
                let sellerSocket = UserFacade.getSocket(listing.sellerId);
                if(sellerSocket){sellerSocket.emit('listing_expired');}
                return Promise.all([
...
function bidOnListing(bidderId, listingId, amount){
    return new Promise((resolve, reject)=>{
        ...
        .spread((valid, maxBidTransaction, listing, amount)=>{
            ...
            if(maxBidTransaction){
                let UserFacade = require("./user.js");
                let s = UserFacade.getSocket(maxBidTransaction.bidderId);
                if(s){
                    s.emit('outbid');
                }
            }
...
function buyoutListing(buyerId, listingId){
    return new Promise((resolve, reject)=>{
        ...
        .then((res)=>{
            resolve({transaction : res[1], listerId: listerId});
        })
        .catch((e)=>{
            Utility.logError(e);
        });
