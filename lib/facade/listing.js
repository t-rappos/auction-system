...
function getAllListings(){
    ...
    return getListingModel().findAll({where:{expiry_date :{$gt:new Date()} }, include: [getItemModel()]});
}

function getExpiredListings(){
    return getListingModel().findAll({where:{expiry_date :{$lte:new Date()} }});
}

function getAccountListings(accountId){
    return getListingModel().findAll({where:{sellerId : accountId},include: [getItemModel()]});
}
...
function oneSecondTimer(){
    ...
    setTimeout(oneSecondTimer,1000);
}
...
function setListingExpired(listingId){
    return getListingModel().update({expiry_date : new Date()},{ where :{id : listingId}});
}
