...
function getAccount(username){
  return new Promise((resolve, reject)=>{
    accountValidator.isUsernameValid(username);
    getAccountModel().findOne({where:{username:username}}).then((res)=>{resolve(res);});
  })
  .catch((e)=>{
      Utility.logError(e);
  });
...
function getAccounts(accountIds){
  return getAccountModel().findAll({where : {id : {in:accountIds}}});
}

//returns list of {name, id}
function getAccountNameIds(accountIds){
  return new Promise((resolve, reject)=>{
    getAccounts(accountIds)
    .then((accounts)=>{
      let userDetails = [];
      accounts.map((acc)=>{userDetails.push({name : acc.username, id : acc.id});});
      resolve(userDetails);
    })
    .catch((e)=>{
      Utility.logError(e);
...
function deleteMessage(messageId){
  return getMessageModel().destroy({where:{id : messageId}});
}
...
module.exports = {
  ...
  getAccountNameIds : getAccountNameIds,
