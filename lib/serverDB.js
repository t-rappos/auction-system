...
function createDBTables(){
  ...
  db.query(createMessageTable, function(done){ if(!done){} });
  db.query(createUsersTable, function(done){ if(!done){} });
};
...
function storeMessages(){
  ...
  db.query(q1, function(done){ if(!done){
    console.log('serverDB: storeMessages: error');} });
  stagedMessages = [];
  }
}
...
function loadMessages(onSuccess){
  ...
  db.query(q, function(result){
     if(!result){
       console.log('serverDB: loadMessages: error');
     }else{
       var msgs = [];
