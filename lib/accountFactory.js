...
function fetchAccount(id){
  if(id == null || id == undefined || id == ''){throw new Error("fetchAccount() id must not be : " + id);}
    ...
    if(isCacheValidFor(id)){
      ...
      .then(function(result){
        if (result && result.rows && result.rows.length > 0){
          ...
          let account = fillAccount(row);
          cacheAccount(account);
          resolve(account);
        } else {
          throw new Error("No account exists for id = " + id);
        }
      })
      .catch((e)=>{
        console.log(e);
...
function createAccount(username, password, email, money){
  return new Promise((resolve,reject)=>{
    ...
    DB.query(Queries.createAccount(username, password, email, money))
    .then((res)=>{
...
function changeAccountParam(account, query){
  return new Promise((resolve,reject)=>{
    ...
    DB.query(query)
    .then(()=>{ return account._refreshFn(); })
    .then(()=>{ /*console.log("changing account param : " + query);*/ resolve(); })
...
function _changeAccountEmail(account, email){
  return new Promise((resolve,reject)=>{
    resolve(
      changeAccountParam(account,Queries.changeEmail(account.getUsername(),email))
    );
  });
}
...
function _changeAccountPassword(account, password){
  return new Promise((resolve,reject)=>{
    resolve(
      changeAccountParam(account,Queries.changePassword(account.getUsername(),password))
    );
  });
}
...
function _addAccountMoney(account, amount){
  return new Promise((resolve,reject)=>{
    resolve(
      changeAccountParam(account,Queries.addMoney(account.getUsername(),amount))
    );
  });
}
...
function _changeAccountMoney(account, money){
  return new Promise((resolve,reject)=>{
    resolve(
      changeAccountParam(account,Queries.changeMoney(account.getUsername(),money))
    );
  });
}
...
function _changeAccountDetails(account, details){
  return new Promise((resolve,reject)=>{
    ...
    resolve(
      changeAccountParam(account,Queries.changeDetails(account.getUsername(),details))
    );
  });
}
...
function _refreshAccount(account){
  return new Promise((resolve, reject)=>{
    fetchAccount(account.getId())
    .then((acc)=>{
      resolve(account.shallowCopy(acc));
    })
    .catch((e)=>{
      console.log(e);
      throw(e);
